...
for ind in parts_org:
    xx = traverseOntoMark(ind, visited_nodes=set())
    if not xx == ';':
       print(str(renderNL(ind))+xx + "<br/>")


def traverseOntoMark(ind0, tabs="&emsp;&emsp;", visited_nodes=set()):
    ...
    if len(triples)==1:
        ...
        if issubclass(prop, owl.ObjectProperty) and (value not in visited_nodes):
            ...
            txt = txt + traverseOntoMark(value, tabs=tabs, visited_nodes=visited_nodes)
        return  txt
    if len(triples) > 1:
        for trip in triples:
            # trip= triples[0]
            ...
            txt = txt + "<br/>" + tabs + ("%s%s%s" % (renderNL(ind0), renderNL(prop), renderNL(value)))
            if issubclass(prop, owl.ObjectProperty) and (value not in visited_nodes):
                txt = txt + traverseOntoMark(value, tabs=tabs+"&emsp;&emsp;", visited_nodes=visited_nodes)
        return txt
    if len(triples) == 0:
        return ";"
